...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'MCQ': return { label: 'Interactive MCQ', color: 'bg-indigo-500/10 text-indigo-300 border-indigo-500/30' };
      case 'Short': return { label: 'Short Note', color: 'bg-violet-500/10 text-violet-300 border-violet-500/30' };
      case 'Essay': return { label: 'Long Answer', color: 'bg-purple-500/10 text-purple-300 border-purple-500/30' };
      case 'Definition': return { label: 'Key Concept', color: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30' };
      case 'FillBlank': return { label: 'Fill-in-Blank', color: 'bg-cyan-500/10 text-cyan-300 border-cyan-500/30' };
      default: return { label: type, color: 'bg-[#111827] text-slate-300 border-[#1E293B]' };
    }
  };
    ...
    <motion.div
      ...
      className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
        question.learned
          ...
          ? 'bg-[#111827]/95 border-amber-500/50 shadow-lg'
          : 'bg-[#111827]/90 border-[#1E293B] hover:border-slate-700 shadow-xl'
      }`}
    >
      {/* Header Bar */}
      <div className="p-4 sm:p-5 space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap text-xs">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-[#0B0F19] text-slate-300 border border-[#1E293B] font-semibold text-[11px]">
              {question.difficulty}
            </span>

            {question.topicTag && (
              <span className="px-2.5 py-0.5 rounded-full bg-[#0B0F19] text-slate-400 text-[11px] border border-[#1E293B]">
                #{question.topicTag}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors border ${
                question.markedForReview
                  ...
                  : 'bg-[#0B0F19] text-slate-400 hover:text-slate-200 border-[#1E293B]'
              }`}
            ...
            <button
              onClick={handleCopy}
              className="px-2.5 py-1 rounded-lg bg-[#0B0F19] hover:bg-[#1E293B] text-slate-300 text-xs font-semibold transition-colors border border-[#1E293B]"
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {question.options.map((opt) => {
                ...
                let optionStyle = 'bg-[#0B0F19] border-[#1E293B] text-slate-200 hover:border-indigo-500/50 hover:bg-[#111827] cursor-pointer';

                if (hasSelected) {
                  if (isCorrectOption) {
                    // Emerald Green for Correct Answer
                    optionStyle = 'bg-emerald-500/20 border-emerald-500 text-emerald-300 font-bold glow-emerald';
                  } else if (isSelected && !isCorrectOption) {
                    // Rose Red for Incorrect Choice
                    optionStyle = 'bg-rose-500/20 border-rose-500 text-rose-300 font-bold glow-rose';
                  } else {
                    // Dimmed options
                    optionStyle = 'bg-[#0B0F19]/40 border-[#0B0F19] text-slate-600 opacity-50 cursor-default';
                  }
                }
            ...
            <AnimatePresence>
              {hasSelected && (
                <motion.div
                  ...
                  className="p-4 rounded-xl bg-[#0B0F19] border border-[#1E293B] space-y-3 mt-3"
                >
        ...
        {!isMCQ && (
          <div className="pt-2">
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-[#0B0F19] hover:bg-[#111827] border border-[#1E293B] text-xs font-bold text-slate-300 flex items-center justify-between transition-colors"
            >
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div
                  ...
                  className="mt-3 p-4 rounded-xl bg-[#0B0F19] border border-[#1E293B] space-y-3 text-xs sm:text-sm"
                >
